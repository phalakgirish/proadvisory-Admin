...
export class AllServicesComponent {
 columnDefinitions = [
     ...
     { def: 'roomType', label: 'Cost', type: 'text', visible: true },
     { def: 'status', label: 'Status', type: 'text', visible: true },
     { def: 'actions', label: 'Actions', type: 'actionBtn', visible: true },
   ];
 
   dataSource = new MatTableDataSource<AllServiceModule>([]);
   selection = new SelectionModel<AllServiceModule>(true, []);
   ...
   constructor(
     ...
     private allPackageService: AllServicesService,
   ...
   openDialog(action: 'add' | 'edit', data?: AllServiceModule) {
   ...
   loadData() {
     this.allPackageService.getAllBookingList().subscribe({
       next: (data: AllServiceModule[]) => {
         this.dataSource.data = data;
         this.isLoading = false;
         this.refreshTable();
         this.dataSource.filterPredicate = (data: AllServiceModule, filter: string) =>
   ...
   onContextMenu(event: MouseEvent, item: AllServiceModule) {
   ...
   deleteItem(row: AllServiceModule){}
   checkOut(row: AllServiceModule){}
   cancelBooking(row: AllServiceModule){}
}
